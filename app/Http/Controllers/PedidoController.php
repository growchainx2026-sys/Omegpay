...
class PedidoController extends Controller
{
    ...
    protected static function paymentPix(Request $request, $data)
    {
        ...
        if ($pedido) {
            $produto = Produto::find($data['produto']['id']);
            $user = $produto->user;
            $payload = [
                'user' => $user,
                'token' => $user->clientId,
            ...
            $responseData = json_decode($transaction->content(), true);
            $httpStatus = $transaction->getStatusCode();

            if ($httpStatus >= 400 || ($responseData['status'] ?? null) === 'error') {
                Log::error('[PEDIDO][PIX] Falha ao criar depósito: ' . json_encode($responseData));
                $pedido->delete();
                return response()->json([
                    'status' => false,
                    'message' => $responseData['message'] ?? $responseData['error'] ?? 'Erro ao gerar PIX. Verifique a configuração do adquirente.',
                ], 422);
            }

            $idTransaction = $responseData['idTransaction'] ?? null;
            if (!$idTransaction) {
                Log::error('[PEDIDO][PIX] Resposta sem idTransaction: ' . json_encode($responseData));
                $pedido->delete();
                return response()->json([
                    'status' => false,
                    'message' => 'Erro ao gerar PIX. Resposta inválida do adquirente.',
                ], 422);
            }
            ...
            if (!$transacaoUnica) {
                ...
                return response()->json([
                    'status' => false,
                    'message' => 'Transação não encontrada após criação.',
                ], 404);
            }
