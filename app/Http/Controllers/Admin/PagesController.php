...
class PagesController extends Controller
{
    ...
    public function extrato(Request $request)
    {
        ...
        $tipo     = $request->input('tipo', 'todos'); // todos, entradas, saidas
        $cliente  = $request->input('cliente'); // user_id
        ...
        if ($tipo === 'todos' || $tipo === 'entradas') {
            $q = TransactionIn::with('user')
                ->where('status', 'pago')
                ->whereBetween('created_at', [$inicio, $fim]);
            if ($cliente) {
                $q->where('user_id', $cliente);
            }
            $entradas = $q->get()->map(function ($row) {
                $taxa = (float) ($row->taxa_cash_in ?? 0);
                $valor = (float) $row->amount;
                $liquido = (float) ($row->cash_in_liquido ?? $valor - $taxa);
                return (object)[
                    'data'    => $row->created_at,
                    'tipo'    => 'entrada',
                    'cliente' => $row->user->name ?? '—',
                    'valor'   => $valor,
                    'taxa'    => $taxa,
                    'liquido' => $liquido,
        ...
        if ($tipo === 'todos' || $tipo === 'saidas') {
            $q = TransactionOut::with('user')
                ->where('status', 'pago')
                ->whereBetween('created_at', [$inicio, $fim]);
            if ($cliente) {
                $q->where('user_id', $cliente);
            }
            $saidas = $q->get()->map(function ($row) {
                return (object)[
                    'data'    => $row->created_at,
                    'tipo'    => 'saida',
                    'cliente' => $row->user->name ?? '—',
        ...
        $movimentos = $entradas->concat($saidas)->sortByDesc('data')->values();

        $users = User::orderBy('name')->get(['id', 'name', 'email']);

        return view('pages.admin.extrato', compact('movimentos', 'users', 'periodo', 'tipo', 'cliente', 'start', 'end'));
    }
