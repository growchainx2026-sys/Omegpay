...
use App\Traits\{CashtimeTrait, SixxpaymentsTrait, EfiTrait, GetpayTrait, Getpay2Trait, XGateTrait, RapdynTrait};
...
class SaqueController extends Controller
{
    ...
    public function indexAprovar(Request $request)
    {
        ...
        // Busca todos os saques pendentes da plataforma web
        $saques = TransactionOut::where('status', 'pendente')
    ...
    {
        $id = $request->input('id');
        $master_password = $request->input('master_password');
        ...
        if (!$master_password || $adminUser->pass_admin !== $master_password) {
            return back()->with("error", "Senha master incorreta!");
        }

        $setting = Setting::first();
        $cashout = TransactionOut::find($id);
        // Usa o adquirente da própria transação ou o global
        $adquirencia = $cashout?->adquirente_ref
            ? strtolower(trim($cashout->adquirente_ref))
            : strtolower(trim($setting->adquirencia ?? ''));

        // Lista de adquirentes que NÃO suportam cash_out automatizado
        $adquirentesSemCashOut = ['pagarme', 'pagar.me'];
        
        if (in_array($adquirencia, $adquirentesSemCashOut)) {
            return redirect()->back()->with('error', 'Este adquirente não suporta aprovação automática. Use a "Aprovação Manual".');
        }
        ...
        switch ($adquirencia) {
            ...
            case 'efi':
             ...
             case 'witetec':
                ...
                break;
            case 'getpay':
                $response = GetpayTrait::liberarSaqueManualGetpay($id);
                break;
            case 'getpay2':
                $response = Getpay2Trait::liberarSaqueManualGetpay2($id);
                break;
            case 'xgate':
                $response = XGateTrait::liberarSaqueManualXgate($id);
                break;
            case 'rapdyn':
                $response = RapdynTrait::liberarSaqueManualRapdyn($id);
                break;
            default:
    ...
    public function aprovarManual(Request $request)
    {
        $id = $request->input('id');
        $master_password = $request->input('master_password');
        ...
        return redirect()->back()->with('success', 'Saque aprovado manualmente com sucesso!');
    }
    ...
    public function addsaida(Request $request)
    {
        ...
        $request->validate([
            'saque_id'        => 'required|exists:users,id',
            'saque_amount'    => 'required|numeric|min:0.01',
            'saque_descricao' => 'required|string|max:191',
        ...
        $pixcashout = [
            ...
            "callbackUrl"           => 'web',
            "descricao_transacao"   => $data['saque_descricao']
        ];
