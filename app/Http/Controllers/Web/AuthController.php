...
class AuthController extends Controller
{
    public function index(Request $request)
    {
        ...
        $autoLoginToken = $request->cookie('auto_login_token');
        $autoLoginEmail = $request->cookie('auto_login_email');
        
        if ($autoLoginToken && $autoLoginEmail) {
            try {
                // Tentar autenticar com o token salvo
                JWTAuth::setToken($autoLoginToken);
                $user = JWTAuth::authenticate();
                
                if ($user && $user->email === $autoLoginEmail) {
                    // Atualizar cookie de token para manter sessão ativa
                    $cookies = [
                        cookie('token', $autoLoginToken, 60 * 24 * 30), // 30 dias
                    ];
                    
                    // Verificar status do usuário
                    if ($user->status === 'aguardando' || $user->status === 'analise') {
                        $response = redirect()->route('auth.enviar-docs');
                ...
                // Token inválido ou expirado, limpar cookies e continuar para login
                $cookies = [
                    cookie()->forget('auto_login_token'),
                    cookie()->forget('auto_login_email'),
                ];
    ...
    public function login(Request $request)
    {
        ...
        // Cookie padrão do token (1 hora)
        $cookieMinutes = 60;
        
        // Se "entrar automaticamente" estiver marcado, criar cookie de longa duração (30 dias)
        if ($request->has('auto_login') && $request->auto_login == '1') {
            $cookieMinutes = 60 * 24 * 30; // 30 dias
            $cookies[] = cookie('auto_login_token', $token, $cookieMinutes);
            $cookies[] = cookie('auto_login_email', $request->email, $cookieMinutes);
        }
        
        // Se "lembrar senha" estiver marcado, salvar email em cookie (30 dias)
        if ($request->has('remember_email') && $request->remember_email == '1') {
            $cookies[] = cookie('remembered_email', $request->email, 60 * 24 * 30); // 30 dias
        } else {
            // Se não marcou, remover cookie se existir
            $cookies[] = cookie()->forget('remembered_email');
        }

        $cookies[] = cookie('token', $token, $cookieMinutes);
    ...
    public function logout(Request $request)
    {
        try {
            ...
            $cookies = [
                ...
                cookie()->forget('auto_login_token'),
                cookie()->forget('auto_login_email'),
                // Não remover remembered_email para manter o email preenchido
            ];
    ...
    public function register(Request $request)
    {
        ...
        // Verificação extra: checar se o e-mail é real/existente usando serviço externo
        /** @var EmailVerificationService $emailVerifier */
        $emailVerifier = app(EmailVerificationService::class);

        if (!$emailVerifier->verify($data['email'])) {
            return redirect()->back()
                ->withErrors(['email' => 'Este e-mail parece inválido ou inexistente.'])
                ->withInput();
        }
        ...
        $payload = collect($data)
            ...
            ->except(['tipo', 'password_confirm'])
            ->filter(fn ($v, $k) => in_array($k, (new User)->getFillable()))
            ->toArray();
        ...
        try {
            ...
            if ($e->getCode() === '23000' || str_contains($e->getMessage(), 'Duplicate')) {
                $msg = 'Email, CPF/CNPJ ou telefone já estão em uso. Use outros dados ou faça login.';
            }
