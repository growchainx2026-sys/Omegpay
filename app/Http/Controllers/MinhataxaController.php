...
class MinhataxaController extends Controller
{
    public function index()
    {
        ...
        if ($setting && $setting->adquirencia_card == 'efi') {
            $efi = Efi::first();
            if ($efi) {
                $taxa_cartao_fixa = $efi->card_tx_fixed ?? 0;
                $taxa_cartao_percent = $efi->card_tx_percent ?? 0;
            }
        } elseif ($setting && $setting->adquirencia_card == 'pagarme') {
            $pagarme = Pagarme::first();
            if ($pagarme) {
                $tx = "1x";
                $taxa_cartao_fixa = 0;
                $taxa_cartao_percent = $pagarme->$tx ?? 0;
            }
        }
