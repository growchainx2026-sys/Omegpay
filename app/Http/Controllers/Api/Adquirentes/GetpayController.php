...
class GetpayController extends Controller
{
    ...
    {
        $data = $request->all();
        Log::debug('[CALLBACK][DEPOSIT][GETPAY] dados recebidos: ' . json_encode($data));
        ...
        if ($getpay?->webhook_token_deposit) {
            ...
            if ($token !== $getpay->webhook_token_deposit) {
                Log::warning('[CALLBACK][DEPOSIT][GETPAY] Token de webhook inválido');
        ...
        if (!$idTransaction) {
            Log::warning('[CALLBACK][DEPOSIT][GETPAY] idTransaction não encontrado no payload');
        ...
        if ($status === 'paid' || $status === 'completed' || $status === 'pago' || $status === 'PAID') {
            ...
            $processa->deposit($idTransaction, 'pago', $end2end, 'GETPAY');
        }
    ...
    {
        $data = $request->all();
        Log::debug('[CALLBACK][WITHDRAW][GETPAY] dados recebidos: ' . json_encode($data));

        $getpay = Getpay::first();
        if ($getpay?->webhook_token_withdraw) {
            $token = $request->header('X-Webhook-Token') ?? $request->input('token') ?? $request->input('webhook_token');
            if ($token !== $getpay->webhook_token_withdraw) {
                Log::warning('[CALLBACK][WITHDRAW][GETPAY] Token de webhook inválido');
                return response()->json(['error' => 'Unauthorized'], 401);
            }
        }
        ...
        if (!$idTransaction) {
            Log::warning('[CALLBACK][WITHDRAW][GETPAY] idTransaction não encontrado no payload');
            return response()->json([], 200);
        }
        ...
        $processa = new ProcessaCallback();
        $processa->withdraw($idTransaction, $status, null, 'GETPAY');
