...
class SettingController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        $allowedRestore = ['logo_light', 'favicon_light', 'login_background'];
        $restoreDefault = $request->input('restore_default');
        if ($restoreDefault) {
            $fields = is_array($restoreDefault) ? $restoreDefault : [$restoreDefault];
            foreach ($fields as $field) {
                if (in_array($field, $allowedRestore) && (Schema::hasColumn('settings', $field) || in_array($field, ['logo_light', 'favicon_light']))) {
                    $data[$field] = '';
                }
            }
        }
