...
use App\Models\{User, Getpay, Setting, TransactionIn, TransactionOut};
use App\Helpers\Helper;

trait GetpayTrait
{
    protected static function getGetpayCredentials(): ?Getpay
    {
        $getpay = Getpay::first();
    ...
    public static function requestDepositGetpay($request)
    {
        $getpay = self::getGetpayCredentials();
        if (!$getpay) {
            return [
                'data' => ['status' => 'error', 'message' => 'GetPay não configurado'],
                'status' => 401,
            ];
        }
        ...
        Log::debug('[GETPAY][DEPOSIT] Request: ' . $url . ' | Response: ' . $response->status() . ' ' . $response->body());
        ...
        if ($response->successful() || $response->status() === 202) {
            ...
            $cashin = [
                ...
                'adquirente_ref' => 'getpay',
                'taxa_cash_in' => $taxa_cash_in,
                'executor_ordem' => 'getpay',
                'request_ip' => $ip,
        ...
        return [
            'data' => [
                'status' => 'error',
                'message' => $response->json('message') ?? $response->json('error') ?? 'Erro ao criar depósito GetPay',
            ],
            'status' => $response->status() ?: 500,
        ];
    }

    public static function requestPaymentGetpay($request)
    {
        ...
        if ($request->baasPostbackUrl === 'web') {
            return self::generateTransactionPaymentManualGetpay($request, $taxa_cash_out, $cashout_liquido, Carbon::now(), $descricao, $user);
        }
        ...
        Log::debug('[GETPAY][WITHDRAW] Request: ' . $url . ' | Response: ' . $response->status() . ' ' . $response->body());
        ...
        return [
            ...
            'data' => [
                'status' => 'error',
                'message' => $response->json('message') ?? $response->json('error') ?? 'Erro ao criar saque GetPay',
            ],
        ];
    }

    protected static function generateTransactionPaymentManualGetpay($request, $taxa_cash_out, $cashout_liquido, $date, $descricao, $user): array
    {
    ...
    public static function liberarSaqueManualGetpay($id)
    {
        $getpay = self::getGetpayCredentials();
        if (!$getpay) {
            return back()->with('error', 'GetPay não configurado.');
        }
