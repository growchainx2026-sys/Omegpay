...
use App\Models\{User, Sixxpayments, Setting, TransactionIn, TransactionOut};
use Faker\Factory as FakerFactory;
use App\Helpers\Helper;

trait SixxpaymentsTrait
{
    ...
    protected static function generateCredentialsSixxpayments()
    {

        $setting = Sixxpayments::first();
        if (!$setting) {
            return false;
        }
    ...
    public static function requestDepositSixxpayments($data)
    {
        if (self::generateCredentialsSixxpayments()) {
            $client_ip = $data->ip();
            ...
            $payload = [
                "postbackUrl"   => url("api/sixxpayments/callback/deposit"),
                "paymentMethod" => "pix",
                "customer"      => [
            ...
            if ($response->successful()) {
                ...
                $cashin = [
                    ...
                    "adquirente_ref"                => 'sixxpayments',
                    "taxa_cash_in"                  => $taxa_cash_in,
                    "executor_ordem"                => 'sixxpayments',
                    "request_ip"                    => $ip,
    ...
    public static function requestPaymentSixxpayments($request)
    {
        ...
        if ($request->baasPostbackUrl === 'web') {
            return self::generateTransactionPaymentManualSixxpayments($request, $taxa_cash_out, $cashout_liquido, $date, $descricao, $user);
        }

        if (self::generateCredentialsSixxpayments()) {
            $callback = url("api/sixxpayments/callback/withdraw");
    ...
    protected static function generateTransactionPaymentManualSixxpayments($request, $taxa_cash_out, $cashout_liquido, $date, $descricao, $user)
    {
    ...
    public static function liberarSaqueManualSixxpayments($id)
    {
        if (self::generateCredentialsSixxpayments()) {
            ...
            if ($response->successful()) {
                ...
                $responseData = $response->json();
                return back()->with('error', $responseData['message'] ?? 'Houve um erro ao liberar saque.');
            }
        }
    }
}
