@php
    ...
    $max = ($nivel_atual && isset($nivel_atual->max)) ? $nivel_atual->max : 1;
    $atual = auth()->user()->transactions_in->where('status', 'pago')->sum('cash_in_liquido');

    // Cálculo da porcentagem entre min e max, evitando divisão por zero
    if ($max <= $min) {
        // Se não houver intervalo válido, considera 0% ou 100% conforme regra de negócio desejada
        $progresso = 0;
    } else {
...
@endphp
<div class="nv-topbar" style="max-width: 400px;position: absolute;top: 16%;right: 240px;" >
