...
@section('content')
            ...
            <table class="table" id="table-admin-depositos">
                ...
                <tbody>
                    @foreach ($depositos->sortByDesc('created_at') as $deposito)
                        <tr class="row-descricao-deposito" data-descricao="{{ e($deposito->descricao_transacao ?? '—') }}" data-valor="R$ {{ number_format($deposito->amount, 2, ',', '.') }}" data-cliente="{{ e($deposito->user->name ?? '—') }}" data-data="{{ $deposito->created_at->format('d/m/Y H:i') }}" style="cursor: pointer;">
                            ...
                            <td>
                                ...
                                @elseif ($deposito->status === 'revisao')
                                    ...
                                    <i data-bs-toggle="modal" data-bs-target="#info-revisao-deposito-{{ $deposito->id }}"
                                        data-lucide="info" class="me-2" onclick="event.stopPropagation();"
                            ...
                            <td>
                                @if ($deposito->status === 'pago' && $deposito?->callback?->status == 'falhou')
                                    ...
                                    <i id="resend-{{ $deposito->callback->id }}" class="fas fa-sync text-info"
                                        style="cursor:pointer;" onclick="event.stopPropagation(); resendWebhook('{{ $deposito->callback->id }}')"></i>
    ...
    <div class="modal fade" id="modalDescricaoDeposito" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Descrição do depósito</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <p class="mb-1 small text-muted"><span id="md-dep-data"></span> · <span id="md-dep-valor"></span> · <span id="md-dep-cliente"></span></p>
                    <p class="mb-0"><strong>Descrição:</strong></p>
                    <div class="p-2 mt-1 rounded bg-light" id="md-dep-descricao">—</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            $('#table-admin-depositos').on('click', 'tbody tr.row-descricao-deposito', function (e) {
                if ($(e.target).closest('button, a, [onclick]').length) return;
                var tr = $(this);
                $('#md-dep-data').text(tr.data('data') || '—');
                $('#md-dep-valor').text(tr.data('valor') || '—');
                $('#md-dep-cliente').text(tr.data('cliente') || '—');
                $('#md-dep-descricao').text(tr.data('descricao') || '—');
                new bootstrap.Modal(document.getElementById('modalDescricaoDeposito')).show();
            });
