...
export default function ProductPage({ setting, produto, checkout, vendedor, utmfy }: ProductPageProps) {
  ...
  const { setVendedor, setSetting, setCheckout, setProduto, setDepoimentos, depoimentos, setMetaAds, meta_ads, setFbq, setUtmfy } = useConfig();
  ...
  return (
    <>
      <Head title={produto?.name || 'Produto'}>
        ...
        <meta property="og:image" content={`/storage/${produto?.image || '/product-placeholder.svg'}`} />
      ...
      >
        <Grid
          ...
          overflowX={'hidden'}
        >
          <GridItem colSpan={12} overflowX={'hidden'}>
            <ProductMain />
          </GridItem>
