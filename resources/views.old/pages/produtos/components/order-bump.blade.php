...
<div class="row">
    <div class="col-lg-4">
        <h4 class="texto-branco">Order bump</h4>
        <p class="texto-branco">Adcione até 5 order bump ao seu produto</p>
    </div>
                ...
                <div class="row">
                    @if($produto->bumps->count() > 0)
                        @foreach($produto->bumps as $bump)
                            <div class="col-12 mb-3" style="display:flex;align-items:center;justify-content:space-between;border-radius: 8px;border: 1px solid var(--gateway-primary-color)">
                                <p class="text-branco pt-3">{{ $bump->product_name }}{{' - '}}de <span style="color: red !important;text-decoration: line-through;">R$ {{ number_format($bump->valor_de, 2, ',','.')}}</span>{{ " " }}por <span style="color: green !important;font-weight:bold !important;">R$ {{ number_format($bump->valor_por, 2, ',','.')}}</span></p>
                                <i class="fa-solid fa-trash text-danger cursor-pointer" onclick="delBump('{{ $bump->id }}')"></i>
                            </div>
                        ...
                        @endforeach
                    @else
                    <div class="col-12"><p>Nenhum Order Bump adcionado</p></div>
                    @endif
                    @if($produto->bumps->count() < 5)
                    <div class="col-12 text-start d-block-inline">
                        <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#addBumpModal">Adicionar</button>&nbsp;<span>{{$produto->bumps->count()}}/5</span>
                    </div>
                    @endif
                </div>
            </div>
