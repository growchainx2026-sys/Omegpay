@php
    ...
    $isUser = auth()->user()->permission == 'user';
@endphp
    ...
    <style>
        :root {
            --gateway-primary-color:
                {{ $isUser ? auth()->user()->software_color : $setting->software_color ?? '#008f39' }}
            ;
            --gateway-background-color:
                {{ $isUser ? auth()->user()->software_color_background : $setting->software_color_background }}
            ;
            --gateway-sidebar-color:
                {{ $isUser ? auth()->user()->software_color_sidebar : $setting->software_color_sidebar }}
            ;
            --gateway-text-color:
                {{ $isUser ? auth()->user()->software_color_text : $setting->software_color_text }}
            ;
            ...
            ;
            --gateway-logo:
                {{ $isUser ? auth()->user()->software_logo : $setting->software_logo }}
            ;
            --bs-btn-close-color:
                {{ $isUser ? auth()->user()->software_color_text : $setting->software_color_text }}
            ;
...
<body>
    ...
    <div class="wrapper" style="height:100vh;position:relative;">
        @include('partials.sidebar')

        <div class="main">
            @include('partials.navbar')
            <div class="alert alert-primary alert-dismissible fade show p-3 accept-fcm d-none" role="alert"
                style="height: 60px;">
                <strong class="mt-1 text-center">Você deseja receber notificações de vendas e saques
                    realizados?</strong>&nbsp;<button id="btn-nft-enable" onclick="initFirebaseMessagingRegistration()"
                    class="btn btn-info text-white btn-xs"
                    style="height: 35px;background:transparent!important;">Aceitar</button>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function (ev) {
            ...
            let messageNotify = document.querySelector('.accept-fcm');
            (async () => {
                const { id: deviceId, source } = await getOrCreateDeviceId();
                if (devices.includes(deviceId)) {
                    return;
                } else {
                    messageNotify.classList.toggle('d-none');
                }
            })()
